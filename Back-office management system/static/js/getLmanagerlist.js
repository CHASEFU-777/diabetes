    ...
    success: function (res) {
      ...
      $.each(res.data, function (i, item) {
        ...
        if(item.grade=='3'){item.grade='低级管理员'}
          ...
          var str = '<tr><td align="center">'+(i+1+(nowpage-1)*10)+'</td><td align="center">'+item.username+'</td><td align="center">'+item.date+'</td><td align="center">低级管理员</td><td align="center">'+item.is_delete+'</td><td><button type="button" onclick="change('+item.id+')" id="btn_change" name="btn_change" class="btn btn-primary"  style="margin-left: 40px;"  >'+a+'</button></td></tr>'
          rows.push(str)
     ...
     if(nowpage == "2"){document.getElementById("pageM").innerHTML='<li><a href="check-Lmanagers.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>';
      document.getElementById("pageM").innerHTML+='<li><a href="check-Lmanagers.html?'+(nowpage+2)+'">'+(nowpage+2)+'</a></li>'}
      if(nowpage == totalpage){document.getElementById("pageM").innerHTML='<li><a href="check-Lmanagers.html?'+(nowpage-1)+'">'+(nowpage-1)+'</a></li>';
     }
     if(nowpage == totalpage1){document.getElementById("pageM").innerHTML='<li><a href="check-Lmanagers.html?'+(nowpage-1)+'">'+(nowpage-1)+'</a></li>';
      document.getElementById("pageM").innerHTML+='<li><a href="check-Lmanagers.html?'+nowpage+'">'+nowpage+'</a></li>'}
     if(nowpage == totalpage2){document.getElementById("pageM").innerHTML='<li><a href="check-Lmanagers.html?'+nowpage+'">'+nowpage+'</a></li>';
      document.getElementById("pageM").innerHTML+='<li><a href="check-Lmanagers.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>'}
     if(nowpage>="3" && nowpage <= totalpage3){document.getElementById("pageM").innerHTML='<li><a href="check-Lmanagers.html?'+nowpage+'">'+nowpage+'</a></li>';
      document.getElementById("pageM").innerHTML+='<li><a href="check-Lmanagers.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>';
      document.getElementById("pageM").innerHTML+='<li><a href="check-Lmanagers.html?'+(nowpage+2)+'">'+(nowpage+2)+'</a></li>'}
      document.getElementById("pageL").innerHTML='<li><a href="check-Lmanagers.html?'+totalpage+'">'+totalpage+'</a></li>'
      $('#last').click(function(){if(nowpage == 1){window.location.href=""}else{
        window.location.href="check-Lmanagers.html?"+(nowpage-1)}
      })
...
$('#btn_new').click(function(){
  if(localStorage.getItem("grade") =='3'){alert('抱歉，您权限不足！');window.location.href="check-Lmanagers.html"}else{
  $.ajax({
      type:'GET',
      ...
      success(res){
          ...
          document.getElementById("nusername").value ='Low_manager'+res.message
...
$.ajax({
  ...
  method: 'POST',
  url: 'http://81.70.0.177:3007/my/search_manager_Low',
  headers: {
    'Authorization': localStorage.getItem('itheima No1. ^_^')
},
