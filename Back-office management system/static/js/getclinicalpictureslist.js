...
function getuserList() {
    // alert(localStorage.getItem('itheima No1. ^_^'))
      var page = location.search.substring(1);
        $.ajax({
          ...
          method: 'POST',
          url: `http://127.0.0.1:8080/api/clinicalpicture?page=${page}`,
        //   headers: {
        ...
        // },
          success: function (res) {
            ...
            $.each(res.data, function (i, item) {
              ...
              var str = '<tr><td align="center">'+(i+1+(nowpage-1)*3)+'</td><td align="center">'+item.Type+'</td><td align="center">'+item.Manifestations+'</td><td align="center">'+item.Instructions+'</td><td align="center"><a href="change-users.html?'+item.id+'" class="change" data-id="'+item.id+'">修改</a>&nbsp;&nbsp;&nbsp;<a href="javascript:;" class="del" data-id="'+item.id+'">'+a+'</a></td></tr>'
              rows.push(str)
            })
           ...
           if(nowpage == "2"){document.getElementById("pageM").innerHTML='<li><a href="check-clinicalpictures.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>';
            document.getElementById("pageM").innerHTML+='<li><a href="check-clinicalpictures.html?'+(nowpage+2)+'">'+(nowpage+2)+'</a></li>'}
            if(nowpage == totalpage){document.getElementById("pageM").innerHTML='<li><a href="check-clinicalpictures.html?'+(nowpage-1)+'">'+(nowpage-1)+'</a></li>';
           }
           if(nowpage == totalpage1){document.getElementById("pageM").innerHTML='<li><a href="check-clinicalpictures.html?'+(nowpage-1)+'">'+(nowpage-1)+'</a></li>';
            document.getElementById("pageM").innerHTML+='<li><a href="check-clinicalpictures.html?'+nowpage+'">'+nowpage+'</a></li>'}
           if(nowpage == totalpage2){document.getElementById("pageM").innerHTML='<li><a href="check-clinicalpictures.html?'+nowpage+'">'+nowpage+'</a></li>';
            document.getElementById("pageM").innerHTML+='<li><a href="check-clinicalpictures.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>'}
           if(nowpage>="3" && nowpage <= totalpage3){document.getElementById("pageM").innerHTML='<li><a href="check-clinicalpictures.html?'+nowpage+'">'+nowpage+'</a></li>';
            document.getElementById("pageM").innerHTML+='<li><a href="check-clinicalpictures.html?'+(nowpage+1)+'">'+(nowpage+1)+'</a></li>';
            document.getElementById("pageM").innerHTML+='<li><a href="check-clinicalpictures.html?'+(nowpage+2)+'">'+(nowpage+2)+'</a></li>'}
            document.getElementById("pageL").innerHTML='<li><a href="check-clinicalpictures.html?'+totalpage+'">'+totalpage+'</a></li>'
            $('#last').click(function(){if(nowpage == 1){window.location.href=""}else{
              window.location.href="check-clinicalpictures.html?"+(nowpage-1)}
            })
            $('#next').click(function(){if(nowpage == totalpage){window.location.href=""}else{
              window.location.href="check-clinicalpictures.html?"+(nowpage+1)}
            })
            $('#go').click(function(){var gopage=$('#select-page').val();
            window.location.href="check-clinicalpictures.html?"+gopage})
    ...
    $('tbody').on('click', '.del', function () {
      ...
      $.ajax({
        method:'GET',
        url:'http://81.70.0.177:8080/my/users/deleteuser/'+id,
